...
def type_1_partition(input, partition_size):  
    B, C, T, V = input.shape
...
def type_1_reverse(partitions, original_size, partition_size):
    T, V = original_size
...
def type_2_partition(input, partition_size):  
    B, C, T, V = input.shape
...
def type_2_reverse(partitions, original_size, partition_size):  
    T, V = original_size
    B = int(partitions.shape[0] / (T * V / partition_size[0] / partition_size[1]))
    output = partitions.view(B, T // partition_size[0], V // partition_size[1], partition_size[0], partition_size[1], -1)
...
def type_3_partition(input, partition_size):  
    B, C, T, V = input.shape
...
def type_3_reverse(partitions, original_size, partition_size): 
    T, V = original_size
    B = int(partitions.shape[0] / (T * V / partition_size[0] / partition_size[1]))
    output = partitions.view(B, T // partition_size[0], V // partition_size[1], partition_size[0], partition_size[1], -1)
...
def type_4_partition(input, partition_size):  
    B, C, T, V = input.shape
...
def type_4_reverse(partitions, original_size, partition_size): 
    T, V = original_size
    B = int(partitions.shape[0] / (T * V / partition_size[0] / partition_size[1]))
    output = partitions.view(B, T // partition_size[0], V // partition_size[1], partition_size[0], partition_size[1], -1)
...
class SkateFormerBlock(nn.Module):
    ...
    def forward(self, input):
        ...
        for i in range(len(self.partition_function)):
            C_attn = split_f_attn[i].shape[1]
            input_partitioned = self.partition_function[i](split_f_attn[i], self.partition_size[i])
            input_partitioned = input_partitioned.view(-1, self.partition_size[i][0] * self.partition_size[i][1], C_attn)
...
class SkateFormer(nn.Module):
    def __init__(
        ...
        # Stem
        stem = []
        stem.append(nn.Conv2d(in_channels=in_channels, out_channels=2 * in_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)))
        stem.append(act_layer())
        stem.append(nn.Conv2d(in_channels=2 * in_channels, out_channels=3 * in_channels, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)))
        stem.append(act_layer())
        stem.append(nn.Conv2d(in_channels=3 * in_channels, out_channels=embed_dim, kernel_size=(1, 1), stride=(1, 1), padding=(0, 0)))
        ...
        self.stages = nn.ModuleList([
            SkateFormerStage(
                ...
                mlp_ratio=mlp_ratio,
                act_layer=act_layer,
                norm_layer_transformer=norm_layer
    ...
    def forward(self, input, index_t=None):
        B, C, T, V, M = input.shape
        output = input.permute(0, 1, 2, 4, 3).contiguous().view(B, C, T, V * M)
        for layer in self.stem:
            output = layer(output)
        if self.index_t:
            te = torch.zeros(B, T, self.embed_dim, device=output.device)
            div_term = torch.exp((torch.arange(0, self.embed_dim, 2, dtype=torch.float, device=output.device) * -(math.log(10000.0) / self.embed_dim)))
        ...
        features = self.forward_features(output) 
        if self.global_pool == 'avg':
            pooled = features.mean(dim=(2, 3))
        elif self.global_pool == 'max':
            pooled = torch.amax(features, dim=(2, 3))
